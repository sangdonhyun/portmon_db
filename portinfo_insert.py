...
class Port:
	def __init__(self, index, slot, number, crc, rx, tx,throughput):
		self.index = index
		self.slot = slot
		self.number = number
		self.crc = crc
	...
	def getData(self):
		...
		return [self.index, self.slot, self.number, self.crc, self.rx, self.tx, self.throughput]
	...
	for switch in pmap.keys():
		...
		for index in indexmap.keys():
			...
			if "tx" in portinfo.keys():
				port = Port(index, portinfo["slot"], portinfo["portnum"], portinfo["crcerr"], int(round(float(portinfo["rx"]))), int(round(float(portinfo["tx"]))), int(round(float(portinfo["throughput"]))))
			else:
				port = Port(index, portinfo["slot"], portinfo["portnum"], portinfo["crcerr"], -1, -1, -1)
		...
		#Query prepare
		query = """insert into perfom_swi.perform_swi_brocade_port_status_""" + today + """(store_time, event_time, swi_serial, data) values('{}', '{}', '{}', '{}') returning swi_serial""".format(store_time,event_time,swiCon.serial,swiDataString)
		print query
		#Set Date format
